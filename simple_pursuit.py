...
global left_d, right_d


# pure pursuit steering angle calc
def calc_pursuit_angle(goal_x, goal_y):
...
def calc_point_pos(_range, angle):
    x1 = _range * math.cos(angle)
    y1 = _range * math.sin(angle)
    return x1, y1


def get_distance(ranges, angles):
    ...
    if len(ranges) > 50:
        ...
        for t in tmp:
            point = Point()
            point.x, point.y = calc_point_pos(ranges[t], angles[t])
...
def get_angle(ranges, angles):
    global marker_points, left_d, right_d
    if len(ranges) > 50:
...
def follow_simple(data):
    # data: single message from topic /scan
    # desired_trajetory: desired distance to the right wall [meters]
    global pubst1, marker_points, prev_steering_err, prev_velocity, point_base_link_frame
    ...
    target_distance = get_distance(data.ranges, angles)
    target_angle, left_d, right_d = get_angle(data.ranges, angles)
    ...
    try:
        steering_err = calc_pursuit_angle(point_base_link_frame.point.x, point_base_link_frame.point.y)
    except:
        steering_err = calc_pursuit_angle(1, -1)
        rospy.loginfo("err")
...
def callback_laser(data):
    ...
    error_steering, velocity = follow_simple(data)
